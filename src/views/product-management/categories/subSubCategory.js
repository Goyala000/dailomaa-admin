...
export default function SubSubCategoryView() {
    ...
    return (
        <MainCard title="Sub Sub Category Form">
            <Box component="form" noValidate sx={{ display: 'grid', gridAutoFlow: 'row', gap: 2 }}>
                <div>
                    <div style={{ marginBottom: '20px' }}>
                        <TextField fullWidth id="outlined-basic" label="Sub Sub Category Name" variant="outlined" />
                    </div>
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'space-between' }}>
                        ...
                        <FormControl sx={{ width: 600 }}>
                            <InputLabel id="demo-multiple-chip-label">Sub Category Name</InputLabel>
                            <Select
                                labelId="demo-multiple-chip-label"
                                id="demo-multiple-chip"
                                multiple
                                value={personName}
                                onChange={handleChange}
                                input={<OutlinedInput id="select-multiple-chip" label="Sub Category Name" />}
                                renderValue={(selected) => (
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                        {selected.map((value) => (
                                            <Chip key={value} label={value} />
